...
import connectDB, { closeConnection } from './config/database-fallback';
import { cache } from './utils/cache';

dotenv.config();

// Connect to MongoDB with fallback configuration
...
app.get('/', (req, res) => {
  res.json({
    name: 'DeFi APY Server (Fallback Mode)',
    version: '1.0.0',
...
const server = app.listen(PORT, () => {
  console.log(`🚀 DeFi APY Server running on port ${PORT} (Fallback Mode)`);
